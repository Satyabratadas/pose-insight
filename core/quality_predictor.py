...
class QualityPredictor:
    def __init__(self, model_path="models/random_forest_quality.pkl", seq_len=30):
        self.seq_len = seq_len
        self.buffer = deque(maxlen=seq_len)
        self.last_prediction = "Analyzing..."
    ...
    def update(self, features):
        ...
        feature_vector = [
            ...
            float(features.get("left_hip") or 0.0),
            float(features.get("right_hip") or 0.0),
            float(features.get("left_elbow") or 0.0),
            float(features.get("right_elbow") or 0.0),
            float(features.get("trunk") or 0.0),
            float(features.get("hip_y") or 0.0),
        ]
        ...
        # RF was trained on flattened 30 x 8 = 240 features
        x = sequence.reshape(1, -1)

        pred = self.model.predict(x)[0]

        self.last_prediction = CLASS_NAMES[int(pred)]
        return self.last_prediction
